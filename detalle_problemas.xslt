...
>
	...
	<xsl:key name="data_type" match="trouble/row/@Description" use="'description'"/>
	<xsl:key name="data_type" match="trouble/row/@AccountName" use="'description'"/>

	<xsl:key name="data_type" match="trouble/row/@TransDate" use="'date'"/>
	<xsl:key name="data_type" match="trouble/row/@PostDate" use="'date'"/>

	<xsl:key name="data_type" match="trouble//@Debit" use="'money'"/>
	<xsl:key name="data_type" match="trouble//@Credit" use="'money'"/>
	<xsl:key name="data_type" match="trouble//@Balance" use="'money'"/>
	<xsl:key name="data_type" match="trouble//@Amount" use="'money'"/>

	<xsl:key name="facts" match="//trouble/row/@*[.!='' and namespace-uri()='']" use="name()"/>
	<xsl:key name="data" match="//trouble/row/@*[.!='' and namespace-uri()='']" use="concat(generate-id(),'::',name())"/>

	<xsl:key name="data" match="/model/trouble/row" use="@acc"/>
	<xsl:key name="data" match="/model/trouble/row" use="@Account"/>
	<xsl:key name="data" match="/model/trouble/row" use="'*'"/>

	<xsl:key name="data:group" match="model/account/row/@key" use="name(../..)"/>
	<xsl:key name="data:group" match="model/trouble[not(row/@xsi:type)]" use="'*'"/>

	<xsl:key name="x-dimension" match="//trouble/@*[namespace-uri()='']" use="name(..)"/>
	<xsl:key name="y-dimension" match="//trouble/*" use="name(..)"/>
	...
	<xsl:template mode="datagrid:aggregate" match="@upce" priority="1">
		<xsl:value-of select="ancestor::trouble[1]/@state:avg_upce"/>
	</xsl:template>
