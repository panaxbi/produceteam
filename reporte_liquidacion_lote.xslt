...
xmlns:section="http://widgets.panaxbi.com/datagrid/section"
...
>
	...
	<xsl:key name="x-dimension" match="//ventas[not(row/@xsi:type)]/@section:*" use="name(..)"/>
	...
	<xsl:template match="@total:upce">
		...
		<xsl:variable name="rows" select="$data/ancestor-or-self::*[1][@qty and @prd]"/>
		<xsl:value-of select="sum($rows/@prd) div sum($rows/@qty)"/>
	</xsl:template>
	...
	<xsl:key name="section" match="ventas/@section:*" use="."/>
	<xsl:key name="section" match="concepto/row" use="'charges'"/>

	<xsl:key name="sections" match="datagrid:header/@*[namespace-uri()='']" use="'*'"/>
	<xsl:key name="sections" match="datagrid:header/@*[namespace-uri()='']" use="name()"/>
	...
	<xsl:template mode="datagrid:header-row" match="*">
		...
		<xsl:for-each select="descendant::datagrid:header">
			<tr style="text-align: center;">
				<th scope="col"></th>
				<xsl:for-each select="key('sections','*')">
					<th scope="col" colspan="{count(key('section', name()))}">
						<xsl:value-of select="."/>
					</th>
				</xsl:for-each>
			</tr>
	...
	<xsl:template mode="datagrid:group-header-cell" match="@charges">
	...
	<xsl:template mode="datagrid:header-cell" match="@charges">
		<xsl:variable name="classification" select="key('classification', name())"/>
		<xsl:variable name="fields" select="//concepto/row/@id"/>
	...
	<xsl:template mode="datagrid:cell" match="@cmm|@netp|@grwr|@liq|@ltb">
		<xsl:param name="row" select="ancestor-or-self::*[1]"/>
		<xsl:param name="data" select="$row/@*"/>
		<td xo-scope="inherit" xo-slot="{local-name()}" class="text-nowrap cell domain-{local-name()}">
			<xsl:call-template name="format">
				<xsl:with-param name="value">
					<xsl:value-of select="sum($data[name()=name(current())])"/>
				</xsl:with-param>
			</xsl:call-template>
		</td>
	</xsl:template>

	<xsl:template mode="datagrid:cell" match="@cmma">
		<xsl:param name="row" select="ancestor-or-self::*[1]"/>
		<xsl:param name="data" select="$row/@*"/>
		<td xo-scope="inherit" xo-slot="{local-name()}" class="text-nowrap cell domain-{local-name()}">
			<xsl:call-template name="format">
	...
	<xsl:template mode="datagrid:cell" match="@charges">
		<xsl:param name="row" select="ancestor-or-self::*[1]"/>
		<xsl:param name="data" select="$row/@*"/>
		<xsl:variable name="classification" select="key('classification', name())"/>
		<xsl:variable name="fields" select="//concepto/row/@id"/>
		<xsl:for-each select="$fields">
			<td xo-scope="inherit" xo-slot="{local-name()}" class="text-nowrap cell domain-cnp-{.}">
				<xsl:call-template name="format">
					<xsl:with-param name="value">
						<xsl:value-of select="sum($row/row[@cnp=current()]/@amt)"/>
					</xsl:with-param>
				</xsl:call-template>
			</td>
		</xsl:for-each>
	...
	<xsl:template mode="datagrid:footer-cell" match="@cmm|@cmma">
		<xsl:param name="rows" select="ancestor-or-self::*[1]"/>
		<xsl:param name="data" select="$rows/@*"/>		
		<td xo-scope="inherit" xo-slot="{local-name()}" class="text-nowrap cell domain-{local-name()}">
			<xsl:call-template name="format">
				<xsl:with-param name="value">
					<xsl:value-of select="sum($data[name()=name(current())])"/>
	...
	<xsl:template mode="datagrid:footer-cell" match="@charges">
		<xsl:param name="rows" select="ancestor-or-self::*[1]"/>
		<xsl:variable name="classification" select="key('classification', name())"/>
		<xsl:variable name="fields" select="//concepto/row/@id"/>
		<xsl:for-each select="$fields">
			<td xo-scope="inherit" xo-slot="{local-name()}" class="text-nowrap text-center cell domain-{local-name()}">
				<xsl:call-template name="format">
					<xsl:with-param name="value" select="sum($rows//row[@cnp=current()]/@amt)"/>
				</xsl:call-template>
			</td>
		</xsl:for-each>
		<th class="text-center">
			<xsl:call-template name="format">
				<xsl:with-param name="value">
					<xsl:value-of select="sum($rows//row/@amt)"/>
				</xsl:with-param>
			</xsl:call-template>
		</th>
	</xsl:template>
